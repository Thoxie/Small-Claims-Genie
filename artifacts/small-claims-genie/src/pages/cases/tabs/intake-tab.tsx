...
export function IntakeTab({
  ...
  const getInitialStep = (): StepNum => {
    ...
    // Clamp to 7: step 8 means "navigate to E-File & Serve", not a rendered step
    if (fromSession >= 1 && fromSession <= 8) return Math.min(fromSession, 7) as StepNum;
  ...
  // isAdvancing covers the gap between mutateAsync resolving (isPending→false) and
  // the React render that shows the new step — keeping all Next buttons disabled
  // throughout the full transition so a fast second click can never sneak through.
  ...
  const handleNext = async (data: Record<string, unknown>) => {
    // Bail out immediately if another advance is already in flight.
    // isAdvancing covers the gap between mutateAsync resolving (isPending→false)
    // and the React render showing the new step, so no second click can slip in.
    ...
    const next = Math.min(currentStep + 1, 8) as StepNum;

    // Step 7 → 8: validate required fields, save as complete, navigate to E-File & Serve
    if (next === 8) {
      const merged: Record<string, unknown> = { ...initialData, ...data };
      ...
      setMissingWarnings([]);
      try {
        await saveIntake.mutateAsync({ id: caseId, data: { step: 8, intakeComplete: true } });
        if (activeTabRef.current === currentStep) {
          invalidateAll();
          navigate(`/cases/${caseId}/efile-serve`);
        }
      } catch (err) {
    ...
    try {
      ...
      if (activeTabRef.current === currentStep) {
        setActiveTab(next);
        window.scrollTo({ top: 0, behavior: "smooth" });
        // Only invalidate after a confirmed advance, not on every save attempt.
        invalidateAll();
      }
    } catch (err) {
      toast({ title: "Could not save progress", description: (err as Error)?.message || "Please check your connection and try again.", variant: "destructive" });
