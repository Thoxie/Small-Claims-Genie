/**
 * End-to-end test: IL Proof of Service form download (unsigned + signed)
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:il-proof-of-service
 *
 * Notes:
...
const TEST_USER_ID       = "test-il-proof-of-service-e2e";
...
async function assertPdf(
  ...
  if (disp.includes("IL-Proof-of-Service")) {
...
async function main() {
  ...
  const [row] = await db.insert(casesTable).values({
    userId: TEST_USER_ID,
    title: "IL Proof of Service E2E Test",
  ...
  const r1 = await assertPdf(
    "unsigned",
    `${BASE_URL}/api/cases/${caseId}/forms/il/proof-of-service`,
    token1, {}, 1000,
  );
  ...
  const r2 = await assertPdf(
    "signed",
    `${BASE_URL}/api/cases/${caseId}/forms/il/proof-of-service/signed`,
    token2,
  ...
  if (passed) {
    console.log("\n✅ All assertions passed — IL Proof of Service PDF correct (unsigned + signed).");
  } else {
    console.error("\n❌ Test failed.");
    process.exit(1);
  }
