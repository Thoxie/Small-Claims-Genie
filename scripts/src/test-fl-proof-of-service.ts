/**
 * End-to-end test: FL Proof of Service form download (unsigned + signed)
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-proof-of-service
 *
 * Notes:
 * - The FL courts website is not accessible from this environment.
...
const TEST_USER_ID = "test-fl-proof-of-service-e2e";
...
async function assertPdf(
  ...
  if (disp.includes("FL-Proof-of-Service")) {
...
async function main() {
  ...
  const [row] = await db.insert(casesTable).values({
    userId: TEST_USER_ID,
    title: "FL Proof of Service E2E Test",
    jurisdictionState: "FL",
    countyId: "fl-orange",
    plaintiffName: "Maria Garcia",
    plaintiffAddress: "789 Lake Shore Dr",
    plaintiffCity: "Orlando",
    plaintiffState: "FL",
    plaintiffZip: "32801",
    defendantName: "Sunshine Contractors LLC",
    defendantAddress: "321 Business Pkwy",
    defendantCity: "Orlando",
    defendantState: "FL",
    defendantZip: "32803",
    claimAmount: 4200,
    claimType: "services",
    claimDescription: "Defendant failed to complete roofing repair work paid in advance.",
  ...
  const r1 = await assertPdf(
    "unsigned",
    `${BASE_URL}/api/cases/${caseId}/forms/fl/proof-of-service`,
    token1, {}, 1000,
  );
  ...
  const r2 = await assertPdf(
    "signed",
    `${BASE_URL}/api/cases/${caseId}/forms/fl/proof-of-service/signed`,
    token2,
  ...
  if (passed) {
    console.log("\n✅ All assertions passed — FL Proof of Service PDF correct (unsigned + signed).");
  } else {
    console.error("\n❌ Test failed.");
    process.exit(1);
  }
