/**
 * End-to-end test: FL Orange County Plain Statement of Claim signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-plain-soc-orange-signed
 *
 * What this tests:
 ...
 *   - Calls POST /api/cases/:id/forms/fl/plain-soc-orange/signed?token=...
...
// ─── Test data ────────────────────────────────────────────────────────────────
const TEST_USER_ID       = "test-fl-plain-soc-orange-signed-e2e";
const EXPECTED_PLAINTIFF = "Angela Thompson";
const EXPECTED_DEFENDANT = "Orlando Home Services LLC";
const EXPECTED_AMOUNT    = "3,000.00";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-plain-soc-orange-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Orange County Plain SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-orange",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "1001 Sand Lake Rd",
      plaintiffCity:    "Orlando",
      plaintiffState:   "FL",
      plaintiffZip:     "32809",
      plaintiffPhone:   "407-555-0133",
      plaintiffEmail:   "angela.thompson@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "2500 Orange Blossom Trail",
      ...
      claimType:        "services",
      claimAmount:      3000,
      claimDescription: "Defendant never completed the work promised under a signed contract for home repairs. Plaintiff paid a deposit of $3,000 and defendant abandoned the job after two days without completing the agreed scope.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/plain-soc-orange/signed?token=${token}`;
    ...
    await assertSignaturePlaced(pdfBuf, SIG_PAGE, SIG_X, SIG_Y, SIG_W, SIG_H, "PLAIN-SOC-ORANGE");

    console.log("\n✅ All assertions passed — FL Orange County Plain SOC signed PDF is correct.");
