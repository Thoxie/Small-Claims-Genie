...
export async function buildSC100AcroformPdf(
  ...
  for (const spec of directQueue) {
    ...
    if (!page) continue;
    // Single-line fields are 11–13pt tall; large text areas are 86pt+.
    // For small fields: baseline ~1–3pt above spec.y (the printed line).
    // For tall fields: baseline starts near the top and wraps downward.
    // Formula naturally handles both: small h → clamps to 1; large h → spec.h−10.
    const startY = spec.y + Math.max(1, spec.h - 10);
