/**
 * End-to-end test: FL Volusia County CL-219 (programmatic SOC) signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-cl219-volusia-soc-signed
 *
 * What this tests:
 ...
 *   - Calls POST /api/cases/:id/forms/fl/cl219-volusia/signed?token=...
 ...
 *
 * Note: CL-219-VOLUSIA delegates to buildFLStatementOfClaim whose signature y-position
 * is computed dynamically from content layout. No pixel-level placement check is
 * performed; HTTP + text-layer assertions catch rendering regressions.
 *
 * This is the programmatic SOC endpoint (fl/cl219-volusia/signed).
 * The template-PDF endpoint (fl/cl219-volusia-pdf/signed) is covered separately.
 */
...
const TEST_USER_ID        = "test-fl-cl219-volusia-soc-signed-e2e";
const EXPECTED_PLAINTIFF  = "James Thornton";
const EXPECTED_DEFENDANT  = "Daytona Plumbing Services LLC";
const EXPECTED_AMOUNT     = "950.00";
const EXPECTED_DESC_CHUNK = "Defendant overcharged for services";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-cl219-volusia-soc-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Volusia CL-219 SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-volusia",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "310 S Beach St",
      plaintiffCity:    "Daytona Beach",
      plaintiffState:   "FL",
      plaintiffZip:     "32114",
      plaintiffPhone:   "386-555-0133",
      plaintiffEmail:   "james.thornton@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "900 Beville Rd",
      defendantCity:    "Daytona Beach",
      defendantState:   "FL",
      defendantZip:     "32119",
      defendantPhone:   "386-555-0244",

      claimType:        "services",
      claimAmount:      950,
      claimDescription: "Defendant overcharged for services not authorized under the original estimate. Plaintiff paid under protest and is seeking reimbursement of the excess amount charged.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/cl219-volusia/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Volusia CL-219 SOC signed PDF is correct.");
  } finally {
    if (caseId !== null) {
