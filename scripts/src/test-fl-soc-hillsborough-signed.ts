 ...
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-hillsborough-signed
 *
 * What this tests:
 ...
 *   - Calls POST /api/cases/:id/forms/fl/soc-hillsborough/signed?token=...
 ...
 *
 * Signature coords (pdf-lib, y from bottom): x=346, y=582, w=180, h=36, page=2
 */
...
// ─── Test data ────────────────────────────────────────────────────────────────
const TEST_USER_ID        = "test-fl-soc-hillsborough-signed-e2e";
...
// The Hillsborough SOC signature is on PAGE 2
const SIG_PAGE = 2;
const SIG_X    = 346;
const SIG_Y    = 582;
const SIG_W    = 180;
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-hillsborough-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/soc-hillsborough/signed?token=${token}`;
    ...
    await assertSignaturePlaced(pdfBuf, SIG_PAGE, SIG_X, SIG_Y, SIG_W, SIG_H, "SOC-HILLSBOROUGH");
