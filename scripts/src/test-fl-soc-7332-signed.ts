/**
 * End-to-end sigcheck test: FL Form 7.332 Work Done / Materials Furnished signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7332-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with workDoneLaborMaterials,
 *   workDoneStartDate, workDoneEndDate set (claimType "Work Done / Materials Furnished").
 ...
 *       labor_materials_credits, work_start_date, work_end_date
...
const TEST_USER          = "test-fl-soc-7332-signed-e2e";
const EXPECTED_PLAINTIFF = "Carlos Menendez";
const EXPECTED_DEFENDANT = "Brickell Builders LLC";
const EXPECTED_AMOUNT    = "8,500.00";
const EXPECTED_LABOR     = "Framing labor 40hrs @ $75, Drywall materials $2,500";
const EXPECTED_START     = "03/01/2025";
const EXPECTED_END       = "04/15/2025";
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7332-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7332-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7332-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.332 Work Done / Materials Furnished — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with work-done fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.332 Work Done Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-broward",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "500 W Broward Blvd",
      plaintiffCity:        "Fort Lauderdale",
      plaintiffState:       "FL",
      plaintiffZip:         "33312",
      plaintiffPhone:       "954-555-0111",
      plaintiffEmail:       "carlos.menendez@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "1200 S Andrews Ave",
      defendantCity:        "Fort Lauderdale",
      defendantState:       "FL",
      defendantZip:         "33316",
      defendantPhone:       "954-555-0222",

      claimType:            "Work Done / Materials Furnished",
      claimAmount:          8500,
      claimDescription:     "Defendant failed to pay for completed construction work.",
      incidentDate:         "2025-04-15",

      workDoneLaborMaterials: EXPECTED_LABOR,
      workDoneStartDate:      "2025-03-01",
      workDoneEndDate:        "2025-04-15",
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_LABOR,     `labor_materials_credits "${EXPECTED_LABOR}"`],
        [EXPECTED_START,     `work_start_date "${EXPECTED_START}"`],
        [EXPECTED_END,       `work_end_date "${EXPECTED_END}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7332: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7332: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7332: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.332 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.332 sigcheck tests passed.");
}
