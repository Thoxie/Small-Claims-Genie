...
import { Loader2, CheckCircle2, X, Info, ChevronDown, ChevronUp, ExternalLink, AlertCircle, CalendarIcon } from "lucide-react";
import { format } from "date-fns";
import { sc104FieldsToBody } from "./sc104-utils";

function DateField({ label, id, value, onChange }: {
  label: string; id: string; value: string; onChange: (v: string) => void;
}) {
  const [open, setOpen] = useState(false);
  const date = (() => { const d = new Date(value); return value && !isNaN(d.getTime()) ? d : undefined; })();
  return (
    <div className="w-full">
      <Label htmlFor={id} className="text-[10px] font-medium text-muted-foreground mb-0.5 block">{label}</Label>
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button id={id} type="button" variant="outline" className={`w-full justify-start text-left font-normal text-xs h-8 px-2 ${!date ? "text-muted-foreground" : ""}`}>
            <CalendarIcon className="mr-1.5 h-3 w-3 shrink-0" />
            {date ? format(date, "MMM d, yyyy") : <span>Select date</span>}
          </Button>
        </PopoverTrigger>
        <PopoverContent className="w-auto p-0" align="start">
          <Calendar mode="single" selected={date} onSelect={(d) => { if (d) { onChange(format(d, "MM/dd/yyyy")); setOpen(false); } }} disabled={(d) => d > new Date()} initialFocus />
        </PopoverContent>
      </Popover>
    </div>
  );
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto bg-background px-4 py-4 space-y-4">
          ...
          <section className="space-y-3">
            ...
            <DateField label="Date served" id="serviceDate" value={f("serviceDate")} onChange={set("serviceDate")} />
          </section>
          ...
          <section className="space-y-2">
            ...
            <DateField label="Date signed" id="signDate" value={f("signDate")} onChange={set("signDate")} />
          </section>
