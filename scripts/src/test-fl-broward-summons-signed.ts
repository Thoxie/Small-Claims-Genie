/**
 * End-to-end test: FL Broward County Summons signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-broward-summons-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-broward and full plaintiff/defendant data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/broward-summons/signed?token=...
 ...
 *
 * Note: FL-BROWARD-SUMMONS uses buildFLSummons whose plaintiff signature y-position is
...
const TEST_USER_ID        = "test-fl-broward-summons-signed-e2e";
const EXPECTED_PLAINTIFF  = "Andre Williams";
const EXPECTED_DEFENDANT  = "Coral Springs Property Mgmt Inc";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-broward-summons-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Broward Summons Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-broward",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "2000 W Oakland Park Blvd",
      plaintiffCity:    "Fort Lauderdale",
      plaintiffState:   "FL",
      plaintiffZip:     "33311",
      plaintiffPhone:   "954-555-0177",
      plaintiffEmail:   "andre.williams@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "9000 W Sample Rd",
      defendantCity:    "Coral Springs",
      defendantState:   "FL",
      defendantZip:     "33065",
      defendantPhone:   "954-555-0288",

      claimType:        "services",
      claimAmount:      2000,
      claimDescription: "Defendant failed to return security deposit and charged unauthorized fees after plaintiff vacated the rental property in good standing.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/broward-summons/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Broward Summons signed PDF is correct.");
  } finally {
    if (caseId !== null) {
