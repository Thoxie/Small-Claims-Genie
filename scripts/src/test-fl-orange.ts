/**
 * End-to-end test: FL Orange County Plain Statement of Claim download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-orange
 *
 * What this tests:
 ...
 *   - Calls POST /api/cases/:id/forms/fl/orange?token=...
...
// ─── Test data ────────────────────────────────────────────────────────────────
const TEST_USER_ID       = "test-fl-orange-e2e";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-orange-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Orange County Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-orange",
    ...
    // ── 3. Call the endpoint ─────────────────────────────────────────────────
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/orange?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Orange County PDF is correct.");
