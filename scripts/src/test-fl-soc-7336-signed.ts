/**
 * End-to-end sigcheck test: FL Form 7.336 Return of Property from Government signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7336-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with claimDescription, incidentDate,
 *   replevinSeizureReason, replevinDemandDate set
 *   (claimType "Return of Property from Government").
 ...
 *       property_description, seizure_reason
...
const TEST_USER             = "test-fl-soc-7336-signed-e2e";
const EXPECTED_PLAINTIFF    = "Diana Reyes";
const EXPECTED_DEFENDANT    = "City of Orlando Police Dept";
const EXPECTED_AMOUNT       = "950.00";
const EXPECTED_PROPERTY     = "Black laptop computer model XPS 15 serial 7A2B3C";
const EXPECTED_SEIZURE_REASON = "Suspected stolen property investigation";

const SIG_PAGE = 1;
const SIG_X    = 220;
const SIG_Y    = 201;   // calibrated from diff bbox: 331x32+220+565 (page height 792)
const SIG_W    = 330;
const SIG_H    = 20;
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7336-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7336-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7336-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.336 Return of Property from Government — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with replevin fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.336 Replevin Govt Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-orange",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "123 Lake Ave",
      plaintiffCity:        "Orlando",
      plaintiffState:       "FL",
      plaintiffZip:         "32801",
      plaintiffPhone:       "407-555-0555",
      plaintiffEmail:       "diana.reyes@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "100 S Hughey Ave",
      defendantCity:        "Orlando",
      defendantState:       "FL",
      defendantZip:         "32801",
      defendantPhone:       "407-555-0666",
      ...
      replevinSeizureReason: EXPECTED_SEIZURE_REASON,
      replevinDemandDate:    "2025-02-01",
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_SEIZURE_REASON,  `seizure_reason "${EXPECTED_SEIZURE_REASON}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7336: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7336: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7336: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.336 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.336 sigcheck tests passed.");
}
