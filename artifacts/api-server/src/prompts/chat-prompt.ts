...
- Step 2 "Make Your Claim" = claim type, amount, incident date, description, how amount calculated, prior demand (date of demand, method of contact, defendant's response), venue basis, eligibility questions, review. Has an AI writing assistant button to help improve the claim description, plus a "Guide Me Through This" guided-questions flow that asks ~9 fact-finding questions tailored to the selected claim type (what happened, what was promised, what went wrong, amount + calculation, evidence, timeline, pre-filing resolution attempts, defendant's response) — it never re-asks anything already entered in Step 1 (names, addresses, court/county/state, business status) and uses no state-specific legal rules, since the app supports California, Florida, Texas, Illinois, and North Carolina.
