...
export async function buildSC100AcroformPdf(
  ...
  for (const spec of directQueue) {
    ...
    if (!page) continue;
    // Baseline sits at most 12pt below the top of the field rect, but always
    // at least 2pt above the bottom edge (the printed line) so text never
    // falls below the line even for short single-line fields.
    const startY = spec.y + Math.max(2, spec.h - 12);
