...
router.post("/cases/:id/forms/sc105/ai-draft", async (req, res): Promise<void> => {
  ...
  if (!rateCheck.allowed) {
    res.status(429).json({ error: `You've used your AI allowance for this hour. Try again in ${Math.ceil((rateCheck.retryAfterSec ?? 3600) / 60)} minutes.`, code: "RATE_LIMITED" });
    return;
  }
