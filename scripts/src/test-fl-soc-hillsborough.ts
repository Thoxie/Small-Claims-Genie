/**
 * End-to-end test: FL Hillsborough County Statement of Claim (alternate county PDF) download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-hillsborough
 *
 * What this tests:
 ...
 *   - Calls POST /api/cases/:id/forms/fl/soc-hillsborough?token=...
 *   - Asserts HTTP 200 + Content-Type: application/pdf + PDF magic bytes + body size
 *   - Uses pdftotext to verify plaintiff name, defendant name, amount, and claim
 *     description text appear in the filled PDF
...
// ─── Test data ────────────────────────────────────────────────────────────────
const TEST_USER_ID        = "test-fl-soc-hillsborough-e2e";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-hillsborough-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Hillsborough County SOC Test Case (auto-cleanup)",
      status:           "draft",
    ...
    // ── 3. Call the endpoint ─────────────────────────────────────────────────
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/soc-hillsborough?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Hillsborough County SOC PDF is correct.");
