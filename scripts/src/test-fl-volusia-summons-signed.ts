/**
 * End-to-end test: FL Volusia County Summons signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-volusia-summons-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-volusia and full plaintiff/defendant data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/volusia-summons/signed?token=...
 ...
 *
 * Note: FL-VOLUSIA-SUMMONS uses buildFLSummons whose plaintiff signature y-position is
...
const TEST_USER_ID        = "test-fl-volusia-summons-signed-e2e";
const EXPECTED_PLAINTIFF  = "Maria Delgado";
const EXPECTED_DEFENDANT  = "Port Orange Roofing LLC";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-volusia-summons-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Volusia Summons Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-volusia",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "120 W Indiana Ave",
      plaintiffCity:    "DeLand",
      plaintiffState:   "FL",
      plaintiffZip:     "32720",
      plaintiffPhone:   "386-555-0155",
      plaintiffEmail:   "maria.delgado@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "4800 Clyde Morris Blvd",
      defendantCity:    "Port Orange",
      defendantState:   "FL",
      defendantZip:     "32129",
      defendantPhone:   "386-555-0266",

      claimType:        "services",
      claimAmount:      3200,
      claimDescription: "Defendant installed a defective roof that leaked within one month of completion. Plaintiff made repeated requests for repairs which were not honored.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/volusia-summons/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Volusia Summons signed PDF is correct.");
  } finally {
    if (caseId !== null) {
