/**
 * End-to-end test: FL Palm Beach County Statement of Claim signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-palm-beach-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-palm-beach, claimType "goods",
 *     and full plaintiff/defendant/contact data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/palm-beach/signed?token=...
 ...
 *
 * Note: FL-PALM-BEACH-SOC delegates to buildFLStatementOfClaim whose signature
 * y-position is computed dynamically from content layout. No pixel-level placement
 * check is performed; HTTP + text-layer assertions catch rendering regressions.
 */
...
const TEST_USER_ID        = "test-fl-palm-beach-signed-e2e";
const EXPECTED_PLAINTIFF  = "Thomas Nguyen";
const EXPECTED_DEFENDANT  = "Boca Raton Electronics Store Inc";
const EXPECTED_AMOUNT     = "650.00";
const EXPECTED_DESC_CHUNK = "Defendant sold a defective television";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-palm-beach-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Palm Beach County SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-palm-beach",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "100 S Dixie Hwy",
      plaintiffCity:    "West Palm Beach",
      plaintiffState:   "FL",
      plaintiffZip:     "33401",
      plaintiffPhone:   "561-555-0177",
      plaintiffEmail:   "thomas.nguyen@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "1800 N Federal Hwy",
      defendantCity:    "Boca Raton",
      defendantState:   "FL",
      defendantZip:     "33432",
      defendantPhone:   "561-555-0288",

      claimType:        "goods",
      claimAmount:      650,
      claimDescription: "Defendant sold a defective television that failed within 30 days of purchase. The store refused to honor the manufacturer's warranty or issue a refund, directing plaintiff to contact the manufacturer directly.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/palm-beach/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Palm Beach County SOC signed PDF is correct.");
  } finally {
    if (caseId !== null) {
