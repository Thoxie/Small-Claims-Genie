...
router.post("/cases/:id/forms/mc030-ai", async (req, res): Promise<void> => {
  ...
  try {
    ...
    // Derive a declaration title and persist title + body text so the E-Filing
    // page download can produce the same pre-filled PDF as Step 6.
    ...
    db.update(casesTable)
      .set({ mc030DeclarationTitle: declarationTitle, mc030DeclarationText: declarationText })
      .where(eq(casesTable.id, id))
      .catch((e: any) => logger.error({ err: e }, "MC-030 title/text save error"));
