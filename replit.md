...
- **Page 1 court-info box calibration** (pixel-verified, 300 DPI form background):
  - Form background: "Superior Court of California, County of" bold at top of fill box (pdf_y≈590 baseline, x=396-579). No pre-printed underlines inside the fill box except one at pdf_y=516.
  - `p1_county`: x=380, y=568 → effective pdf_y=572.5, LEFT-ALIGNED with courthouse fields (was x=475 — changed to avoid centering asymmetry)
  - `p1_courthouse_name`: x=380, y=542 → effective pdf_y=546.5, below county line
  - `p1_courthouse_address`: x=380, y=528 → effective pdf_y=532.5
  - `p1_courthouse_location`: x=380, y=514 → effective pdf_y=518.5, just above pre-printed underline at pdf_y=516
  - `p1_case_name`: x=362, y=442
  - LIFT constant = 4.5 (applied as liftedY = field.y + LIFT in form-renderer.ts); SC100_CONFIG cached at forms.ts line 23 (restart required after sc100.json edits)
- **Page 3 section 3c (How calculated) calibration** (pixel-verified against 300 DPI form bg strip px 560-960):
  - "c. How did you calculate..." form label at pdf_y≈652-658; fill lines at pdf_y≈644, 631, 618
  - `p3_how_calculated`: x=63, y=641 → effective 645.5 → sits on 1st fill line below "c" label; lineH=13, maxLines=3
  - `p3_mc030_check`: x=36, y=586 → condition `{ exists: "needsMC031" }` — only checked when description or calculation overflows
- **MC-031 overflow logic** (enrichForSC100 in forms.ts):
  - `claimDescriptionForForm`: if description > 360 chars, truncates with "… (see MC-030)"; sets `needsMC031=true`
  - `howAmountCalculated`: if > 210 chars, sets `needsMC031=true`
  - Short descriptions shown as-is (no unconditional MC-030 suffix)
- **Pages 2-4**: Plaintiff, defendant, claim, venue, declaration items overlaid with case data
- **Debug workflow**: `curl localhost:18080/api/forms/sc100/debug-preview?mode=debug` → pdftoppm -r 150 → convert to PNG → visually compare crosshairs vs form boxes
