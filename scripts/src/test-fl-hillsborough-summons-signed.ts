/**
 * End-to-end test: FL Hillsborough County Summons signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-hillsborough-summons-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-hillsborough and full plaintiff/defendant data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/hillsborough-summons/signed?token=...
 ...
 *
 * Note: FL-HILLSBOROUGH-SUMMONS uses buildFLSummons whose plaintiff signature y-position
...
const TEST_USER_ID        = "test-fl-hillsborough-summons-signed-e2e";
const EXPECTED_PLAINTIFF  = "Gary Morrison";
const EXPECTED_DEFENDANT  = "Tampa Bay Pool Services Inc";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-hillsborough-summons-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Hillsborough Summons Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-hillsborough",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "700 N Franklin St",
      plaintiffCity:    "Tampa",
      plaintiffState:   "FL",
      plaintiffZip:     "33602",
      plaintiffPhone:   "813-555-0177",
      plaintiffEmail:   "gary.morrison@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "6200 E Hillsborough Ave",
      defendantCity:    "Tampa",
      defendantState:   "FL",
      defendantZip:     "33610",
      defendantPhone:   "813-555-0288",

      claimType:        "services",
      claimAmount:      1600,
      claimDescription: "Defendant damaged the pool equipment during a maintenance visit and refused to pay for the replacement parts required to restore the system to working order.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/hillsborough-summons/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Hillsborough Summons signed PDF is correct.");
  } finally {
    if (caseId !== null) {
