/**
 * End-to-end test: FL Orange County Statement of Claim signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-orange-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-orange, claimType "services",
 *     and full plaintiff/defendant/contact data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/orange/signed?token=...
 ...
 *
 * Note: FL-ORANGE-SOC delegates to buildFLStatementOfClaim whose signature y-position
...
const TEST_USER_ID        = "test-fl-orange-signed-e2e";
const EXPECTED_PLAINTIFF  = "Rachel Kim";
const EXPECTED_DEFENDANT  = "Orlando Event Rentals LLC";
const EXPECTED_AMOUNT     = "800.00";
const EXPECTED_DESC_CHUNK = "Defendant retained the deposit";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-orange-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Orange County SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-orange",
      ...
      plaintiffState:   "FL",
      plaintiffZip:     "32801",
      plaintiffPhone:   "407-555-0133",
      plaintiffEmail:   "rachel.kim@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "3200 E Colonial Dr",
      defendantCity:    "Orlando",
      defendantState:   "FL",
      defendantZip:     "32803",
      defendantPhone:   "407-555-0244",

      claimType:        "services",
      claimAmount:      800,
      claimDescription: "Defendant retained the deposit after canceling the event rental agreement without cause. Plaintiff had fulfilled all contract conditions and was entitled to the refund of the deposit paid.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/orange/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Orange County SOC signed PDF is correct.");
  } finally {
    if (caseId !== null) {
