/**
 * End-to-end sigcheck test: FL Form 7.333 Money Lent signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7333-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with claimDescription and incidentDate
 *   (used as loan_date) set (claimType "Money Lent").
 ...
 *     - pdftotext: plaintiff name, defendant name, amount, loan_description, loan_date
...
const TEST_USER            = "test-fl-soc-7333-signed-e2e";
const EXPECTED_PLAINTIFF   = "Patricia Okonkwo";
const EXPECTED_DEFENDANT   = "James Whitfield";
const EXPECTED_AMOUNT      = "2,500.00";
const EXPECTED_DESCRIPTION = "Personal loan for home repair expenses";
const EXPECTED_LOAN_DATE   = "01/20/2025";
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7333-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7333-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7333-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.333 Money Lent — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with money-lent fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.333 Money Lent Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-orange",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "400 W Church St",
      plaintiffCity:        "Orlando",
      plaintiffState:       "FL",
      plaintiffZip:         "32801",
      plaintiffPhone:       "407-555-0111",
      plaintiffEmail:       "patricia.okonkwo@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "800 N Orange Ave",
      defendantCity:        "Orlando",
      defendantState:       "FL",
      defendantZip:         "32801",
      defendantPhone:       "407-555-0222",

      claimType:            "Money Lent",
      claimAmount:          2500,
      claimDescription:     EXPECTED_DESCRIPTION,
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_DESCRIPTION, `loan_description "${EXPECTED_DESCRIPTION}"`],
        [EXPECTED_LOAN_DATE,   `loan_date "${EXPECTED_LOAN_DATE}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7333: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7333: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7333: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.333 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.333 sigcheck tests passed.");
}
