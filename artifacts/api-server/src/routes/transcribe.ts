...
const audioUpload = multer({
  ...
  fileFilter: (_req, file, cb) => {
    ...
    // because MediaRecorder on Chrome/Edge appends them and they are not in the set.
...
router.post("/transcribe", audioUpload.single("audio"), async (req, res): Promise<void> => {
  ...
  try {
    ...
    res.status(500).json({ error: `Voice transcription failed: ${msg}. Please try again or type your message.` });
  }
});
