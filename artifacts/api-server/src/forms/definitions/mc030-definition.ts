...
async function generateMC030Declaration(
  ...
  const prompt = [
    ...
    hasExhibits ? [
      ...
      `  (Exhibit X — [EXACT document name from the list]: [optional: what it proves])`,
      `  CRITICAL: The name after the dash MUST be the EXACT name from the numbered list above.`,
      `  Do NOT invent a different name or describe the content instead of using the list name.`,
      `  Example if list says "Document 2: Check Copy": (Exhibit A — Check Copy: confirms $1,542.42 payment)`,
...
// ─── Form definition & registry registration ─────────────────────────────────
