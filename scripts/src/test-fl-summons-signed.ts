/**
 * End-to-end test: FL Summons (generic statewide) signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-summons-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-leon and full plaintiff/defendant data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/summons/signed?token=...
 ...
 *
 * Note: FL-SUMMONS uses buildFLSummons whose plaintiff signature y-position is computed
 * dynamically from content layout. No pixel-level placement check is performed;
...
const TEST_USER_ID        = "test-fl-summons-signed-e2e";
const EXPECTED_PLAINTIFF  = "Patricia Okonkwo";
const EXPECTED_DEFENDANT  = "Capital City Movers LLC";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-summons-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Summons Generic Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-leon",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "300 S Calhoun St",
      plaintiffCity:    "Tallahassee",
      plaintiffState:   "FL",
      plaintiffZip:     "32301",
      plaintiffPhone:   "850-555-0199",
      plaintiffEmail:   "patricia.okonkwo@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "820 E Park Ave",
      defendantCity:    "Tallahassee",
      defendantState:   "FL",
      defendantZip:     "32301",
      defendantPhone:   "850-555-0288",

      claimType:        "services",
      claimAmount:      1100,
      claimDescription: "Defendant damaged furniture during a move and refused to compensate plaintiff for the documented losses.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/summons/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Summons signed PDF is correct.");
  } finally {
    if (caseId !== null) {
