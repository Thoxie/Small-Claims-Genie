...
import { EFileServePage } from "@/pages/cases/efile-serve-page";
...
// ── Beta claim bridge ─────────────────────────────────────────────────────────
// Fires once per sign-in session to claim a beta slot for the current user.
// The /beta/claim endpoint is idempotent — already-claimed users get a no-op.
// This ensures that any sign-up path (free-trial, modal, pricing page, etc.)
// automatically grants beta access without requiring explicit frontend wiring.
function BetaClaimBridge() {
...
function Router() {
  ...
  if (location.startsWith("/free-trial") || location.startsWith("/beta") || location.startsWith("/startup")) return <><BetaClaimBridge /><Layout><BetaPage /></Layout></>;
  ...
  return (
    <>
      ...
      <Switch>
        ...
        {/* ── E-File / Serve page — must be before /cases/:id ── */}
        <Route path="/cases/:id/efile">
          {(params: { id?: string }) => (
            <RequireAuth><EFileServePage caseIdParam={params.id ?? ""} /></RequireAuth>
          )}
        </Route>
