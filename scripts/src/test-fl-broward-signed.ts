/**
 * End-to-end test: FL Broward County Statement of Claim signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-broward-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-broward, claimType "goods",
 *     and full plaintiff/defendant/contact data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/broward/signed?token=...
 ...
 *
 * Note: FL-BROWARD-SOC delegates to buildFLStatementOfClaim whose signature y-position
...
const TEST_USER_ID        = "test-fl-broward-signed-e2e";
const EXPECTED_PLAINTIFF  = "Kevin Patterson";
const EXPECTED_DEFENDANT  = "Fort Lauderdale Auto Repair Inc";
const EXPECTED_AMOUNT     = "1,500.00";
const EXPECTED_DESC_CHUNK = "Defendant charged for repairs";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-broward-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Broward County SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-broward",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "1234 E Las Olas Blvd",
      plaintiffCity:    "Fort Lauderdale",
      plaintiffState:   "FL",
      plaintiffZip:     "33301",
      plaintiffPhone:   "954-555-0111",
      plaintiffEmail:   "kevin.patterson@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "5678 N Federal Hwy",
      ...
      claimType:        "goods",
      claimAmount:      1500,
      claimDescription: "Defendant charged for repairs that were never performed. Plaintiff paid $1,500 for parts and labor that were billed but not completed. Vehicle was returned in worse condition than when dropped off.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/broward/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Broward County SOC signed PDF is correct.");
  } finally {
    if (caseId !== null) {
