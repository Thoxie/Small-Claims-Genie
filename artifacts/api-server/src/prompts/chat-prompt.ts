...
- Step 2 "Make Your Claim" = claim type, amount, incident date, description, how amount calculated, prior demand (date of demand, method of contact, defendant's response), venue basis, eligibility questions, review. Claim type options: Money Owed, Unpaid Debt, Security Deposit, Property Damage, Vehicle Damage/Accident, Landlord/Tenant Dispute, Online Purchase/Marketplace Dispute, Unpaid Wages/Employment, Contract Dispute, Fraud, Other. Has an AI writing assistant button to help improve the claim description, plus a "Guide Me Through This" guided-questions flow that asks ~9 fact-finding questions tailored to the selected claim type (what happened, what was promised, what went wrong, amount + calculation, evidence, timeline, pre-filing resolution attempts, defendant's response) — it never re-asks anything already entered in Step 1 (names, addresses, court/county/state, business status) and uses no state-specific legal rules, since the app supports California, Florida, Texas, Illinois, and North Carolina. After a guided draft is generated, the app automatically runs an AI check that flags any genuinely missing facts (e.g. no dollar amount, vague dates, no evidence mentioned) so the user can fill gaps before moving on.
...
- Not yet downloaded court forms (CA: SC-100; FL: Statement of Claim; TX: TX Small Claims Petition; IL: IL Small Claims Complaint; NC: AOC-CVM-200 Complaint) → direct to Step 6 "Create Court Forms"
