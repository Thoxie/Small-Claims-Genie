/**
 * End-to-end sigcheck test: FL Form 7.337 Account Stated signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7337-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with claimDescription and incidentDate
 *   (used as account_statement_date) set (claimType "Account Stated").
 ...
 *       account_details, account_statement_date
...
const TEST_USER            = "test-fl-soc-7337-signed-e2e";
const EXPECTED_PLAINTIFF   = "Theresa Nakamura";
const EXPECTED_DEFENDANT   = "Sunrise Catering Services";
const EXPECTED_AMOUNT      = "3,100.00";
const EXPECTED_DETAILS     = "Unpaid invoices for catering services rendered in Q1 2025";
const EXPECTED_STMT_DATE   = "03/31/2025";

const SIG_PAGE = 1;
const SIG_X    = 220;
const SIG_Y    = 297;   // calibrated from diff bbox: 331x32+220+469 (page height 792)
const SIG_W    = 330;
const SIG_H    = 20;
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7337-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7337-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7337-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.337 Account Stated — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with account-stated fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.337 Account Stated Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-palm-beach",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "101 N Olive Ave",
      plaintiffCity:        "West Palm Beach",
      plaintiffState:       "FL",
      plaintiffZip:         "33401",
      plaintiffPhone:       "561-555-0777",
      plaintiffEmail:       "theresa.nakamura@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "500 S Rosemary Ave",
      defendantCity:        "West Palm Beach",
      defendantState:       "FL",
      defendantZip:         "33401",
      defendantPhone:       "561-555-0888",

      claimType:            "Account Stated",
      claimAmount:          3100,
      claimDescription:     EXPECTED_DETAILS,
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_DETAILS,    `account_details "${EXPECTED_DETAILS}"`],
        [EXPECTED_STMT_DATE,  `account_statement_date "${EXPECTED_STMT_DATE}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7337: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7337: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7337: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.337 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.337 sigcheck tests passed.");
}
