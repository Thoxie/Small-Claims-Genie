/**
 * End-to-end sigcheck test: FL Form 7.335 Stolen Property from Pawnbroker signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7335-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with claimDescription, incidentDate,
 *   pawnbrokerLawEnforcementAgency, pawnbrokerReportNumber, and
 *   pawnbrokerWrittenDemandDate set (claimType "Stolen Property from Pawnbroker").
 ...
 *       property_description, law_enforcement_agency, report_number
...
const TEST_USER              = "test-fl-soc-7335-signed-e2e";
const EXPECTED_PLAINTIFF     = "Marcus Johnson";
const EXPECTED_DEFENDANT     = "Gold Exchange Pawnshop";
const EXPECTED_AMOUNT        = "1,200.00";
const EXPECTED_PROPERTY      = "14k gold necklace with diamond pendant";
const EXPECTED_AGENCY        = "Miami-Dade Police Department";
const EXPECTED_REPORT_NUMBER = "PD-2025-004422";
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7335-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7335-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7335-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.335 Stolen Property from Pawnbroker — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with pawnbroker fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.335 Pawnbroker Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-miami-dade",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "750 NW 5th Ave",
      plaintiffCity:        "Miami",
      plaintiffState:       "FL",
      plaintiffZip:         "33128",
      plaintiffPhone:       "305-555-0333",
      plaintiffEmail:       "marcus.johnson@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "2200 SW 8th St",
      defendantCity:        "Miami",
      defendantState:       "FL",
      defendantZip:         "33135",
      defendantPhone:       "305-555-0444",

      claimType:            "Stolen Property from Pawnbroker",
      claimAmount:          1200,
      claimDescription:     EXPECTED_PROPERTY,
      incidentDate:         "2025-02-10",

      pawnbrokerLawEnforcementAgency: EXPECTED_AGENCY,
      pawnbrokerReportNumber:         EXPECTED_REPORT_NUMBER,
      pawnbrokerWrittenDemandDate:    "2025-03-01",
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_PROPERTY,      `property_description "${EXPECTED_PROPERTY}"`],
        [EXPECTED_AGENCY,        `law_enforcement_agency "${EXPECTED_AGENCY}"`],
        [EXPECTED_REPORT_NUMBER, `report_number "${EXPECTED_REPORT_NUMBER}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7335: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7335: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7335: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.335 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.335 sigcheck tests passed.");
}
