/**
 * End-to-end test: FL Miami-Dade CLK-CT-333 signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-clkct333-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-miami-dade, claimType "goods",
 *     and full plaintiff/defendant/contact data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/clkct333/signed?token=...
 ...
 *
 * Signature coords (pdf-lib, y from bottom): x=323, y=235, w=130, h=36, page=1
 */
...
// ─── Test data ────────────────────────────────────────────────────────────────
const TEST_USER_ID       = "test-fl-clkct333-signed-e2e";
const EXPECTED_PLAINTIFF = "Maria Gonzalez";
const EXPECTED_DEFENDANT = "South Florida Contractors LLC";
const EXPECTED_AMOUNT    = "2,400.00";
const EXPECTED_DESC_CHUNK = "Defendant failed to complete";
...
function buildSolidPngDataUrl(
  ...
  // Raw image data: each scanline = filter byte (0) + width*3 RGB bytes
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-clkct333-signed-test-${Date.now()}.pdf`);
  try {
...
async function assertSignaturePlaced(
  ...
  try {
    ...
    try {
      // Get page height from the rendered PNG
      ...
      // Convert pdf-lib (bottom-left origin) to image (top-left origin)
      const imgX = pdfX;
      const imgY = pageH - pdfY - h;
      const crop = `${w}x${h}+${imgX}+${imgY}`;

      // Compute mean pixel brightness (grayscale 0=black, 1=white)
      ...
      // A solid black PNG embedded at coords should make the region mean < 0.98
      assert(
        mean < 0.98,
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Miami-Dade CLK-CT-333 Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-miami-dade",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "1234 Brickell Ave",
      plaintiffCity:    "Miami",
      plaintiffState:   "FL",
      plaintiffZip:     "33131",
      plaintiffPhone:   "305-555-0101",
      plaintiffEmail:   "maria.gonzalez@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "9876 SW 8th St",
      defendantCity:    "Miami",
      defendantState:   "FL",
      defendantZip:     "33144",
      defendantPhone:   "305-555-0202",

      claimType:        "goods",
      claimAmount:      2400,
      claimDescription: "Defendant failed to complete the contracted renovation work despite receiving full payment. Plaintiff made multiple requests for completion or refund, all of which were ignored.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/clkct333/signed?token=${token}`;
    ...
    // ── 9. Pixel-level signature placement check ─────────────────────────────
    console.log("Checking signature pixel placement…");
    await assertSignaturePlaced(pdfBuf, SIG_PAGE, SIG_X, SIG_Y, SIG_W, SIG_H, "CLK-CT-333");

    console.log("\n✅ All assertions passed — FL Miami-Dade CLK-CT-333 signed PDF is correct.");
