...
import { ChevronRight, CheckCircle, FileText, Circle, PartyPopper, X, Sparkles, PlusCircle } from "lucide-react";
...
function CaseView({ c }: { c: Case }) {
  ...
  const [justSaved] = useState(() => {
    const val = sessionStorage.getItem("scg-just-saved") === "true";
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* ── Resume / save-confirmation banner ── */}
      <div className="flex items-center gap-3 bg-green-50 border-2 border-green-500 rounded-xl px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          {justSaved ? (
            ...
            <>
              <p className="text-sm font-semibold text-green-800">Your case is ready.</p>
              {savedDate && (
                <p className="text-xs text-green-700 mt-0.5">Last updated {savedDate}.</p>
              )}
            </>
          )}
        </div>
        <Button
          asChild
          size="lg"
          className="gap-2 rounded-full px-8 py-5 text-base font-bold bg-primary text-primary-foreground hover:bg-primary/90 shrink-0"
        >
          <Link href={`/cases/${c.id}${hash}`}>
            Resume Case <ChevronRight className="h-5 w-5" />
          </Link>
        </Button>
      </div>
      ...
      <div className="bg-white border border-border rounded-xl shadow-sm overflow-hidden">
        ...
        {/* Zone 3 — updated date */}
        <div className="border-t border-gray-100 flex items-center px-6 py-3">
              ...
              <div className="text-center py-16 flex flex-col items-center gap-5">
                ...
                <div>
                  ...
                  <p className="text-muted-foreground text-sm mt-1">
                    Start your first case to begin building your claim.
                  </p>
                </div>
                <Button asChild>
                  <Link href="/cases/new">
