/**
 * End-to-end test: FL Palm Beach County Summons signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-palm-beach-summons-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-palm-beach and full plaintiff/defendant data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/palm-beach-summons/signed?token=...
 ...
 *
 * Note: FL-PALM-BEACH-SUMMONS uses buildFLSummons whose plaintiff signature y-position
...
const TEST_USER_ID        = "test-fl-palm-beach-summons-signed-e2e";
const EXPECTED_PLAINTIFF  = "Sandra Brooks";
const EXPECTED_DEFENDANT  = "West Palm Painting Contractors LLC";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-palm-beach-summons-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Palm Beach Summons Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-palm-beach",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "500 Clematis St",
      plaintiffCity:    "West Palm Beach",
      plaintiffState:   "FL",
      plaintiffZip:     "33401",
      plaintiffPhone:   "561-555-0199",
      plaintiffEmail:   "sandra.brooks@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "2700 N Military Trail",
      ...
      claimType:        "services",
      claimAmount:      1450,
      claimDescription: "Defendant applied the wrong paint color to the exterior of plaintiff's home and refused to redo the work at no charge, demanding additional payment for the error.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/palm-beach-summons/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Palm Beach Summons signed PDF is correct.");
  } finally {
    if (caseId !== null) {
