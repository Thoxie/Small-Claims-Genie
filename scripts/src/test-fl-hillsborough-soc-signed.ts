/**
 * End-to-end test: FL Hillsborough County Statement of Claim signed form download
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-hillsborough-soc-signed
 *
 * What this tests:
 *   - Creates a temporary case with county fl-hillsborough, claimType "services",
 *     and full plaintiff/defendant/contact data
 *   - Creates a download token directly in the DB (bypasses Clerk auth)
 *   - Calls POST /api/cases/:id/forms/fl/hillsborough/signed?token=...
 ...
 *
 * Note: FL-HILLSBOROUGH-SOC delegates to buildFLStatementOfClaim whose signature
...
const TEST_USER_ID        = "test-fl-hillsborough-soc-signed-e2e";
const EXPECTED_PLAINTIFF  = "Natalie Vega";
const EXPECTED_DEFENDANT  = "Tampa Landscaping Services Corp";
const EXPECTED_AMOUNT     = "2,750.00";
const EXPECTED_DESC_CHUNK = "Defendant destroyed the landscaping";
...
async function extractPdfText(pdfBuf: Buffer): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-hillsborough-soc-signed-test-${Date.now()}.pdf`);
  try {
...
async function run() {
  ...
  try {
    ...
    const [inserted] = await db.insert(casesTable).values({
      userId:           TEST_USER_ID,
      title:            "FL Hillsborough County SOC Signed Test Case (auto-cleanup)",
      status:           "draft",
      countyId:         "fl-hillsborough",

      plaintiffName:    EXPECTED_PLAINTIFF,
      plaintiffAddress: "1200 N Ashley Dr",
      plaintiffCity:    "Tampa",
      plaintiffState:   "FL",
      plaintiffZip:     "33602",
      plaintiffPhone:   "813-555-0155",
      plaintiffEmail:   "natalie.vega@example.com",

      defendantName:    EXPECTED_DEFENDANT,
      defendantAddress: "4400 S Dale Mabry Hwy",
      defendantCity:    "Tampa",
      defendantState:   "FL",
      defendantZip:     "33611",
      defendantPhone:   "813-555-0266",

      claimType:        "services",
      claimAmount:      2750,
      claimDescription: "Defendant destroyed the landscaping previously installed by plaintiff's contractor. During unauthorized trimming work, defendant damaged mature trees and garden beds requiring full replacement.",
    ...
    const url = `${BASE_URL}/api/cases/${caseId}/forms/fl/hillsborough/signed?token=${token}`;
    ...
    console.log("\n✅ All assertions passed — FL Hillsborough County SOC signed PDF is correct.");
  } finally {
    if (caseId !== null) {
