/**
 * End-to-end sigcheck test: FL Form 7.334 Promissory Note signed form
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-soc-7334-signed
 *
 * What this tests:
 *   SIGNED SIGCHECK — Creates a case with incidentDate (note_date),
 *   noteInterestDue, noteInterestRate, noteAttorneyFees set
 *   (claimType "Promissory Note").
 ...
 *       note_date, interest_rate, interest_due
...
const TEST_USER            = "test-fl-soc-7334-signed-e2e";
const EXPECTED_PLAINTIFF   = "Sandra Kowalski";
const EXPECTED_DEFENDANT   = "Robert Tanner";
const EXPECTED_AMOUNT      = "5,000.00";
const EXPECTED_NOTE_DATE   = "06/01/2024";
const EXPECTED_INTEREST_RATE = "8.00%";
const EXPECTED_INTEREST_DUE  = "200.00";

const SIG_PAGE = 1;
const SIG_X    = 220;
const SIG_Y    = 175;   // calibrated from diff bbox: 331x32+220+591 (page height 792)
const SIG_W    = 330;
const SIG_H    = 20;
...
async function extractPdfText(pdfBuf: Buffer, label: string): Promise<string> {
  const tmpPdf = join(tmpdir(), `fl-soc-7334-test-${label}-${Date.now()}.pdf`);
  try {
...
async function renderPage(pdfBuf: Buffer, page: number): Promise<{ pngPath: string; width: number; height: number }> {
  ...
  const pdfPath = join(tmpdir(), `fl-soc-7334-${ts}.pdf`);
  const pngBase = join(tmpdir(), `fl-soc-7334-page-${ts}`);
  try {
...
async function testSigned(): Promise<void> {
  console.log("\n=== FL Form 7.334 Promissory Note — Signed Sigcheck ===");
  let caseId: number | null = null;

  try {
    console.log("Creating test case with promissory-note fields…");
    const [inserted] = await db.insert(casesTable).values({
      userId:               TEST_USER,
      title:                "FL Form 7.334 Promissory Note Signed Sigcheck (auto-cleanup)",
      status:               "draft",
      countyId:             "fl-hillsborough",

      plaintiffName:        EXPECTED_PLAINTIFF,
      plaintiffAddress:     "300 N Hyde Park Ave",
      plaintiffCity:        "Tampa",
      plaintiffState:       "FL",
      plaintiffZip:         "33606",
      plaintiffPhone:       "813-555-0111",
      plaintiffEmail:       "sandra.kowalski@example.com",

      defendantName:        EXPECTED_DEFENDANT,
      defendantAddress:     "600 S Dale Mabry Hwy",
      defendantCity:        "Tampa",
      defendantState:       "FL",
      defendantZip:         "33609",
      defendantPhone:       "813-555-0222",

      claimType:            "Promissory Note",
      claimAmount:          5000,
      claimDescription:     "Defendant failed to repay promissory note.",
      incidentDate:         "2024-06-01",

      noteInterestRate:     EXPECTED_INTEREST_RATE,
      noteInterestDue:      EXPECTED_INTEREST_DUE,
      noteAttorneyFees:     "0.00",
    ...
    if (pdfText) {
      const checks: [string, string][] = [
        ...
        [EXPECTED_NOTE_DATE,     `note_date "${EXPECTED_NOTE_DATE}"`],
        [EXPECTED_INTEREST_RATE, `interest_rate "${EXPECTED_INTEREST_RATE}"`],
        [EXPECTED_INTEREST_DUE,  `interest_due "${EXPECTED_INTEREST_DUE}"`],
      ];
    ...
    try {
      ...
      assert(
        bbox !== null,
        `FL-7334: signed page ${SIG_PAGE} is identical to unsigned — signature image was NOT embedded.`,
      );
      ...
      assert(
        dx <= tolPx && dy <= tolPx,
        `FL-7334: signature landed at ${gotCx.toFixed(0)},${gotCy.toFixed(0)} but expected ~${expCx.toFixed(0)},${expCy.toFixed(0)} ` +
      ...
      assert(
        mean < 0.6,
        `FL-7334: added block on page ${SIG_PAGE} is not dark (mean=${mean.toFixed(4)} ≥ 0.6) — signature not rendered solid.`,
      );
    ...
    console.log("\n✅ FL Form 7.334 signed PDF fills and signs correctly.");
...
async function run() {
  await testSigned();
  console.log("\n✅ All FL Form 7.334 sigcheck tests passed.");
}
