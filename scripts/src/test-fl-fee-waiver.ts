/**
 * End-to-end test: FL Fee Waiver form download (unsigned + signed)
 *
 * Run with:
 *   pnpm --filter @workspace/scripts run test:fl-fee-waiver
 *
 * Notes:
 * - FL Application for Determination of Civil Indigent Status (Fla. Stat. § 57.082).
 * - Generated programmatically via pdf-lib (png-overlay technique).
...
const TEST_USER_ID = "test-fl-fee-waiver-e2e";
...
async function assertPdf(
  ...
  if (disp.includes("FL-Fee-Waiver")) {
  ...
  if (buf.length >= expectedSizeBump) {
    console.log(`  ✓ Size ≥ ${expectedSizeBump.toLocaleString()} bytes (≥ expected minimum)`);
  } else {
    console.log(`  ✗ Size ${buf.length} < expected minimum ${expectedSizeBump}`);
    passed = false;
  }
...
async function main() {
  ...
  const [row] = await db.insert(casesTable).values({
    userId: TEST_USER_ID,
    title: "FL Fee Waiver E2E Test",
    jurisdictionState: "FL",
    countyId: "fl-hillsborough",
    plaintiffName: "James Williams",
    plaintiffAddress: "100 Cypress Ave",
    plaintiffCity: "Tampa",
    plaintiffState: "FL",
    plaintiffZip: "33601",
    defendantName: "Bay Area Rentals Inc",
    defendantAddress: "500 Harbor Blvd",
    defendantCity: "Tampa",
    defendantState: "FL",
    defendantZip: "33602",
    claimAmount: 1800,
    claimType: "property",
    claimDescription: "Defendant failed to return security deposit after lease termination.",
  ...
  const r1 = await assertPdf(
    "unsigned",
    `${BASE_URL}/api/cases/${caseId}/forms/fl/fee-waiver`,
    token1, {}, 1000,
  );
  ...
  const r2 = await assertPdf(
    "signed",
    `${BASE_URL}/api/cases/${caseId}/forms/fl/fee-waiver/signed`,
    token2,
  ...
  if (passed) {
    console.log("\n✅ All assertions passed — FL Fee Waiver PDF correct (unsigned + signed).");
  } else {
    console.error("\n❌ Test failed.");
    process.exit(1);
  }
